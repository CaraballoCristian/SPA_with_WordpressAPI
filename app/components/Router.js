...
export async function Router() {
  ...
  if (hash !== "#/" && hash) {
    document.getElementById("root").style.background =
      `radial-gradient(circle at -10% -10%,#ff32c8ab, 25%, transparent 50%),
  ...
  if (!hash || hash === "#/") {
      ...
      await ajax({
        ...
        cbSuccess: (posts) => {
          ...
          for (let i = 1; i < posts.length; i++) {
            $cards.innerHTML += HomeCard(
              posts[i],
              `secondary-card-${i} home-card-secondary`,
            );
          }
    ...
    let $containerLoader, aux;

    if (api.page === 1) {
      $containerLoader = d.createElement("section");
      $containerLoader.classList.add(
        "posts-section",
        "grid-fluid",
        "skeleton-section",
      );
    ...
    let $containerSearchLoader, aux;

    if (api.page === 1 && query) {
      $containerSearchLoader = d.createElement("section");
      $containerSearchLoader.classList.add(
        "search-section",
        "skeleton-section",
      );
    ...
    await ajax({
      ...
      cbSuccess: async (search) => {
        ...
        if (search.length !== 0) {
          $containerSearch.innerHTML = search.map(p => SearchCard(p)).join("");
